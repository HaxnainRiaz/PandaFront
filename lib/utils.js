...
export function resolveImageUrl(src, placeholder = "https://images.unsplash.com/photo-1596755094514-f87e34085b2c?auto=format&fit=crop&q=80&w=1200") {
    ...
    // Handle object structure from Cloudinary/Local upload variants
    if (typeof src === 'object') {
        const variants = src.variants || {};
        const url = variants.large || variants.medium || variants.small || variants.thumb || src.originalUrl || src.url;
    ...
    if (process.env.NODE_ENV === 'production' && imageUrl.includes('localhost:5000')) {
        imageUrl = imageUrl.replace(/https?:\/\/localhost:5000/, getApiBaseUrl());
    }
